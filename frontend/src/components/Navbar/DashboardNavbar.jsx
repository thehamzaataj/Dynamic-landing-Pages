...
const DashboardNavbar = ({ title, subtitle, isDarkMode, ontoggleDarkMode }) => {
      ...
      <div>
        <div className={`flex items-center space-x-1.5 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
          <Link to={"/dashboard"} className="flex items-center">
            <RiHome7Line className="text-[15px] mr-2" />
          </Link>
        ...
        <div className="pt-2">
          <h1 className={`text-${isDarkMode ? 'white' : 'black'} text-[16px] font-bold`}>{title}</h1>
        </div>
      </div>
